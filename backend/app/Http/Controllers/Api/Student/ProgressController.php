...
class ProgressController extends Controller
{
    ...
    public function getRadarStats(Request $request)
    {
        ...
        foreach($answers as $ans) {
            ...
            if(!isset($subjectsStat[$subName])) {
                $subjectsStat[$subName] = ['total' => 0, 'correct' => 0];
            }
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $attemptQuery = ExamAttempt::where('student_id', $studentId)
                ->where('status', 'submitted')
                ->whereBetween('created_at', [$monthStart, $monthEnd]);

            if ($courseId) {
                $attemptQuery->whereHas('exam.subject.courses', function($q) use ($courseId) {
                    $q->where('courses.id', $courseId);
                });
            }

            $avg = $attemptQuery->avg('total_score') ?? 0;
    ...
    public function getLearningPath(Request $request)
    {
        ...
        $enrollments = ClassEnrollment::with(['classes.cohort.course.subjects.topics'])
            ->where('student_id', $studentId)
            ->get();
        ...
        if (!$courseId && $courses->count() > 1) {
            $currentCourseName = 'Tất cả khóa học';
        }
        ...
        foreach($courses as $course) {
            ...
            foreach($course->subjects as $subject) {
                ...
                $subjectsData[] = [
                    ...
                    'name' => $subject->name,
                    'is_passed' => $isPassed,
                    'topics' => $subject->topics->map(fn($t) => ['id' => $t->id, 'name' => $t->name])
                ];
            }
            $tree[] = [
                'id' => $course->id,
        ...
        if ($courseId) {
            $subjectScoresQuery->join('course_subject', 'subjects.id', '=', 'course_subject.subject_id')
        ...
        foreach($answers as $ans) {
            ...
            if(!isset($topicsStat[$topicId])) {
                $topicsStat[$topicId] = [
                    ...
                    'subject_name' => $ans->question->subject->name ?? 'Môn học',
                    'total' => 0,
                    'correct' => 0
                ];
            }
        ...
        foreach($topicsStat as $stat) {
            ...
            if ($rate <= 50 && $stat['total'] >= 1) {
                $weaknesses[] = [
                    ...
                    'correct_rate' => $rate,
                    'suggestion' => 'Ôn tập lại kiến thức cơ bản và cách vận dụng của ' . $stat['topic_name'] . '.'
                ];
            }
        }

        return response()->json([
            'current_course' => $currentCourseName,
